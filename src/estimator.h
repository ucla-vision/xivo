...
private:
  ...
  void ComposeMotion(State &X, const Vec3 &V,
                     const Eigen::Matrix<ftype, 6, 1> &gyro_accel, ftype dt);
  // perform Fehlberg numerical integration
  void Fehlberg(const Vec3 &gyro0, const Vec3 &accel0, ftype dt);
  // perform Prince-Dormand numerical integration
  void PrinceDormand(const Vec3 &gyro0, const Vec3 &accel0, ftype dt);
  // return max(slope), i.e., max(V, max(gyro), max(accel))
  ftype PrinceDormandStep(const Vec3 &gyro0, const Vec3 &accel0, ftype dt);
  // perform vanilla RK4 without step control
  void RK4(const Vec3 &gyro0, const Vec3 &accel0, ftype dt);
  // perform one-step in RK4 integration (4 inner steps)
  void RK4Step(const Vec3 &gyro0, const Vec3 &accel0, ftype dt);
  ...
  std::tuple<ftype, bool> HuberOnInnovation(const Vec2 &inn, ftype Rviz);
...
private:
  ...
  ftype ransac_thresh_, ransac_prob_, ransac_Chi2_;
  bool use_OOS_;                    // Out-Of-State measurement update
  bool use_compression_;            // measurement compression
  ftype compression_trigger_ratio_; // use measurement compression, if the ratio
  ...
  // measurement noise
  ftype init_z_, init_std_x_, init_std_y_, init_std_z_;

  // jacobians
  Eigen::SparseMatrix<ftype> F_, G_;
  // covariances
  MatX P_;
  ...
  ftype Roos_;           // oos measurement covariance in pixels
  ftype R_;              // covariance of visual measurement noise, in pixels
  ftype Rtri_;           // measurement covariance, depth sub-filter
  ftype outlier_thresh_; // outlier threshold -- multipler of the measurement
                         // variance

  // MH (Mahalanobis) gating parameters
  bool use_MH_gating_;
  ftype MH_thresh_;           // MH threshold
  int min_required_inliers_;  // minimal inliers needed to perform update
  ftype MH_thresh_multipler_; // if not enough inliers, repeatedly multiple the
